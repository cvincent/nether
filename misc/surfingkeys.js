...
api.unmap("p")
api.unmap("P")
api.mapkey("pp", "paste URL into current tab", function() {
    api.Clipboard.read(function(response) { document.location = response.data })
})
api.mapkey("Pp", "paste URL into new tab", function() {
    api.Clipboard.read(function(response) { api.tabOpenLink(response.data) })
})
...
// Nord
api.Hints.style('border: solid 2px #4C566A; color:#4C566A; background: initial; background-color: #EBCB8B; font-size: 16px;');
api.Hints.style("border: solid 2px #4C566A !important; padding: 1px !important; color: #E5E9F0 !important; background: #EBCB8B !important; font-size: 16px;", "text");
api.Visual.style('marks', 'background-color: #EBCB8B; color: #4C566A !important;');

...
let
  decodemail = (pkgs.writeScriptBin "decodemail.py" ''
  ...
  watch-maildir = (pkgs.writeShellScriptBin "watch-maildir" ''
    #!/usr/bin/env bash
    ...
    ${pkgs.inotify-tools}/bin/inotifywait -e move --monitor -r ${myHomeDir}/mail | while read line

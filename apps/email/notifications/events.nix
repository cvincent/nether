...
let
  watch-vdirsyncer-dir = (pkgs.writeShellScriptBin "watch-vdirsyncer-dir" ''
    #!/usr/bin/env bash

    PATH=$PATH:/run/current-system/sw/bin

    mkdir -p ${myHomeDir}/.cache/event-notifications
    touch ${myHomeDir}/.cache/event-notifications/next-15m-seen
    touch ${myHomeDir}/.cache/event-notifications/next-1m-seen
    ...
    do
      ...
      seen=$(grep "$uid" ${myHomeDir}/.cache/event-notifications/next-$starts_in-seen)

      if [ "$seen" == "" ]; then
        echo $uid >> ${myHomeDir}/.cache/event-notifications/next-$starts_in-seen
        ${notify-event}/bin/notify-event "$title" "in $starts_in ($start_time)" "$location" & disown
      fi
    done
  '');

  notify-event = (pkgs.writeShellScriptBin "notify-event" ''
    #!/usr/bin/env bash

    ${pkgs.alsa-utils}/bin/aplay ${myHomeDir}/dotfiles/misc/notification.wav &
...
in
{
  ...
  systemd.user.services.clear-send-event-notifications-cache = {
    Install.WantedBy = [ "graphical.target" ];
    Service.ExecStart = "rm ${myHomeDir}/.cache/event-notifications/*";
